...
from utils.professional_resume_builder import ProfessionalResumeBuilder
...
# Cache expensive app initialization
@st.cache_resource(show_spinner=False)
def get_app_instance():
    """Cached app instance to avoid reinitializing builders on every rerun"""
    return ResumeApp()
...
class ResumeApp:
    def __init__(self):
        ...
        self.pro_builder = ProfessionalResumeBuilder()
        ...
        try:
            ...
            import logging
            logging.warning(f"Database initialization failed: {str(e)}. App will run in demo mode.")

        # Load external CSS (cached in session state to avoid reloading)
    ...
    def apply_global_styles(self):
        if 'styles_loaded' in st.session_state:
            return
    ...
    def build_chat_prompt(self, user_input, mode):
        """Build a prompt for the AI chatbot using user resume context."""
        system_lines = [
            "You are an expert career coach and resume advisor.",
            "Answer clearly and provide actionable suggestions.",
            "",
        ]

        if mode == "General Resume Advice":
            system_lines.append("Provide resume improvement tips, structure advice, and content suggestions.")
        elif mode == "Resume Rewrite":
            system_lines.append("Rewrite the user's resume bullet points, summary, or profile text to be stronger, concise, and ATS-friendly.")
        elif mode == "Interview Coaching":
            system_lines.append("Offer interview preparation guidance, sample answers, question strategy, and confidence tips.")
        elif mode == "Job Search Strategy":
            system_lines.append("Help the user target the right job titles, keywords, and career path steps.")

        profile = st.session_state.form_data.get('personal_info', {})
        ...
        if profile.get('full_name'):
            system_lines.append(f"Name: {profile.get('full_name')}")
        if profile.get('title'):
            system_lines.append(f"Current title: {profile.get('title')}")
        if summary:
        ...
        if technical:
            system_lines.append(f"Technical skills: {technical}")
        if soft:
            system_lines.append(f"Soft skills: {soft}")
        if languages:
            system_lines.append(f"Languages: {languages}")
        if tools:
            system_lines.append(f"Tools: {tools}")

        if experiences:
            system_lines.append("Work experience highlights:")
            for exp in experiences[:3]:
                role = exp.get('role') or exp.get('title') or exp.get('position') or ''
                company = exp.get('company') or ''
                if role or company:
                    system_lines.append(f"- {role} at {company}".strip())

        system_lines.append("")
        system_lines.append("If you don't have enough information, ask the user for more context before answering.")
        system_lines.append("")
        system_lines.append(f"User question: {user_input}")
    ...
    def render_ai_chatbot(self):
        ...
        page_header(
            "AI Chatbot",
            "Ask smart questions about resume writing, interview prep, and job search strategy."
        )

        if 'chat_history' not in st.session_state:
            st.session_state.chat_history = []

        if 'ai_chat_mode' not in st.session_state:
            st.session_state.ai_chat_mode = "General Resume Advice"

        st.markdown(
            """
            <div style='padding: 1rem; border-radius: 16px; background: rgba(76, 175, 80, 0.08); margin-bottom: 1rem;'>
                <strong>AI Career Coach:</strong> Ask for resume feedback, bullet rewrites, interview answers, or target job advice.
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        with col2:
            ...
            if st.session_state.form_data.get('summary'):
                st.write(f"**Summary:** {st.session_state.form_data.get('summary')[:120]}...")
            skills = st.session_state.form_data.get('skills_categories', {})
            if skills.get('technical'):
                st.write(f"**Technical skills:** {', '.join(skills.get('technical', [])[:8])}")
            if skills.get('soft'):
                st.write(f"**Soft skills:** {', '.join(skills.get('soft', [])[:8])}")
            st.markdown("---")
            st.write("**Tip:** Provide a sample bullet point or question for the strongest answer.")

        with col1:
            mode = st.selectbox(
                "Assistant Mode",
                ["General Resume Advice", "Resume Rewrite", "Interview Coaching", "Job Search Strategy"],
                key="ai_chat_mode"
            )

            user_message = st.chat_input("Ask your AI career coach anything about your resume or interview.")
            if user_message:
                st.session_state.chat_history.append({"role": "user", "content": user_message})
                prompt = self.build_chat_prompt(user_message, mode)
                try:
                    response_text, provider = LLMOrchestrator().generate_content(prompt)
                    st.session_state.chat_history.append({"role": "assistant", "content": response_text})
                    st.session_state.last_chat_provider = provider
                except Exception as e:
                    st.error(f"AI chatbot error: {str(e)}")
                    st.session_state.chat_history.append({"role": "assistant", "content": "Sorry, I couldn't generate a response right now. Please check your API configuration or try again."})
                    st.session_state.last_chat_provider = None

            button_col1, button_col2 = st.columns([1, 1])
            with button_col1:
                if st.button("Clear chat", key="clear_ai_chat"):
                    st.session_state.chat_history = []
                    if 'last_chat_provider' in st.session_state:
                        del st.session_state.last_chat_provider
                    st.experimental_rerun()

            with button_col2:
                if st.button("Show usage tips", key="show_chat_tips"):
                    st.info("Try asking: 'How can I improve my professional summary?', 'Rewrite this bullet point for ATS', or 'What job titles match my profile?'")
            ...
            if st.session_state.chat_history:
                ...
                if st.session_state.get('last_chat_provider'):
                    st.caption(f"Provider: {st.session_state.last_chat_provider}")
            else:
                st.info("Your AI chat is empty. Type a question in the chat box above to get started.")
    ...
    def render_builder(self):
        ...
        # Enhanced Template Selection with tabs
        col1, col2 = st.columns([3, 1])
        with col1:
            st.markdown('<h3 style="color: var(--primary); margin-bottom: 1rem;">✨ Choose Your Resume Template</h3>', unsafe_allow_html=True)
        
        template_info = {
            "Modern Pro": "Clean and contemporary design with professional color accents",
            "Executive": "Formal traditional style perfect for leadership roles",
            "Minimalist": "Ultra-clean design focusing on content",
            "Corporate": "Professional blue tones for corporate environments",
            "Creative Tech": "Modern tech-focused design with creative elements"
        }
        
        template_options = list(template_info.keys())
        ...
        with col1:
            selected_template = st.selectbox(
                "Resume Template",
                template_options,
                help="Choose a professional template design"
            )
        
        # Show template description
        with col2:
            st.info(f"📋 {template_info[selected_template]}")
        ...
        with col1:
            ...
            title = st.text_input("Professional Title", value=st.session_state.form_data['personal_info'].get('title', ''), help="E.g. Senior Software Engineer, Product Manager")
            email = st.text_input(
    "Email",
    value=existing_email,
     key="email_input")
            ...
            st.session_state.form_data['personal_info']['title'] = title
    ...
    st.session_state.form_data['skills_categories']['tools']),
        ...
        if st.button("Generate Resume 📄", type="primary"):
            ...
            try:
                ...
                try:
                    # Generate resume using the professional template engine
                    resume_buffer = self.pro_builder.generate_resume(resume_data, selected_template)
                    if resume_buffer:
                        try:
